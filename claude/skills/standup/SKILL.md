...
**⚠️ 중요:** 어제 활동이 있었던 task일지라도 **반드시 task의 현재 `진행 상태` 필드를 fetch해서 확인할 것**. 에이전트가 어제 alive였다는 이유만으로 (진행중) 추론하지 말 것.

- **진행 중** → 어제·오늘 양쪽에 등장, sub-bullet 마커 `(진행중)`
- **할 일** → 후보 풀에 포함
- **완료** → 어제 섹션만 `(완료)` 마커, **오늘 섹션에서는 제외**
- **리뷰 중** → 어제·오늘 양쪽 등장, `(리뷰중)`
- **리뷰 대응 필요** (자체 정의) → PR에 reviewer 코멘트 있고 미반영이면 `(리뷰 대응)`

**감지 실패 사례 (2026-06-01):** TASK-4657이 이미 완료됐는데 에이전트 활동 기반으로 (진행중) 추론. 매 standup 실행 시 Won 할당 task의 진행 상태를 fetch하지 않은 게 원인. **이 단계는 옵션이 아닌 필수**.
