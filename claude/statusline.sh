...
jq_full='[
  ...
  (.rate_limits.five_hour.resets_at // "-" | tostring | if . == "" then "-" else . end),
  (.rate_limits.seven_day.used_percentage // null | if . then (. | round | tostring) else "null" end),
  (.rate_limits.seven_day.resets_at // "-" | tostring | if . == "" then "-" else . end),
  (.model.display_name // "-" | if . == "" then "-" else . end),
  (.workspace.current_dir // "-" | if . == "" then "-" else . end)
] | @tsv'
...
IFS="$tab" read -r used_tokens window_size live_session_id live_api_ms live_output_tokens live_five_pct live_five_reset live_seven_pct live_seven_reset model_name cwd <<EOF
$parsed
EOF

# "-" is the jq placeholder for empty fields (bash collapses consecutive tabs)
[ "$live_five_reset" = "-" ] && live_five_reset=""
[ "$live_seven_reset" = "-" ] && live_seven_reset=""
...
if [ -n "$model_name" ]; then
  model_part="${MAGENTA}${model_name}${RESET}"
fi

# Path: worktree paths shown as repo⎇worktree, otherwise ~-abbreviated cwd
path_part=""
if [ -n "$cwd" ]; then
  case "$cwd" in
    */.claude/worktrees/*)
      repo="${cwd%%/.claude/worktrees/*}"
      wt="${cwd#*/.claude/worktrees/}"
      display_path="$(basename "$repo")${YELLOW}⎇${wt%%/*}${RESET}"
      ;;
    *)
      display_path="${cwd/#$HOME/\~}"
      ;;
  esac
  path_part="${BLUE}${display_path}${RESET}"
fi
...
[ -n "$out" ] && out="${out} | "
printf "%b%b | %b | %b\n" "$out" "$context_part" "$five_part" "$seven_part"
