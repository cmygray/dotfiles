...
4. 전사본과 라이브 노트를 Obsidian으로 보존:
   ...
   - 프론트매터 형식:
     ...
     ---
     ```
5. 1단계에서 Slack 채널을 받았으면, Slack 공유물을 먼저 `meeting_notes_share_draft.md`로 작성하고 사용자 검토를 받습니다. 사용자가 "즉시 전송"을 명시하지 않은 한 Slack에 바로 보내지 마세요.
   - 드래프트에는 실제 Slack에 보낼 내용을 **그대로** 적습니다.
   - 첫 메시지: `*[회의 주제]* 회의록` + 라이브 노트 회의 요약 섹션 그대로
   - 스레드 댓글: 라이브 노트 논의 내용 전문
   - 별도 "검토 필요" 섹션을 맨 위에 추가해 전사 오류 가능성이 있는 이름, 날짜, 숫자, 의사결정, 담당자, 액션 아이템을 체크리스트로 표시합니다.
   - 불확실한 내용은 단정하지 말고 `확인 필요:`로 표시합니다. 컨텍스트나 전사에 없는 내용을 보강하지 마세요.
   - Slack 드래프트 기능은 스레드 댓글까지 함께 남기기 어렵기 때문에 기본 타협안은 "로컬 드래프트 검토 → 승인 후 본문과 스레드 댓글 동시 전송"입니다.
   - 사용자가 승인하면 slack_send_message MCP 도구로 전송합니다. 글자수 제한(5000자)을 초과하면 스레드로 분할합니다.
   - 사용자가 Slack 드래프트만 원하면 첫 메시지만 드래프트로 남기고, 스레드 댓글용 전문은 `meeting_notes_share_draft.md`에 유지한다고 안내합니다.
...
- 공유 전 검토 단계에서는 오해 소지가 큰 항목을 우선 확인하세요: 결정사항, 담당자, 날짜, 수치, 동료 발언의 의도, 외부 공유 가능한 내용.
