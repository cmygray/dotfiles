...
4. 전사본과 라이브 노트를 Obsidian으로 보존:
   ...
   - 프론트매터 형식:
     ...
     ---
     ```
5. 1단계에서 Slack 채널을 받았으면, 라이브 노트 내용을 **그대로** Slack에 공유 (slack_send_message MCP 도구 사용). 별도 요약을 만들지 말 것 — 윤색하거나 격상하면 회의 내용이 왜곡됨. 글자수 제한(5000자)을 초과하면 스레드로 분할:
   - 첫 메시지: `*[회의 주제]* 회의록` + 라이브 노트 회의 요약 섹션 그대로
   - 스레드: 라이브 노트 논의 내용 전문
