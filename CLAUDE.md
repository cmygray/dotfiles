# dotfiles — 글로벌 Claude Code 설정 저장소

이 레포는 `~/.claude/` 하위 설정의 source of truth입니다.

## 심볼릭 링크 구조

- `~/.claude/{skills, rules, agents, settings.json, RTK.md}` → `dotfiles/claude/...`
- `~/.claude/hooks/rtk-rewrite.sh`는 글로벌 (dotfiles 외)
...
- `.gitattributes`에 `claude/settings.json filter=strip-claude-local` 등록
