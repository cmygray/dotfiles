...
def pget [] {
    pet search | commandline edit $in
}

def kubefz [] {
    kubectl get secrets -A --field-selector type=Opaque -o jsonpath='{ range .items[*]}{.metadata.name}{"\n"}{end}'
| fzf
| commandline edit $"kubectl get secrets/($in) --template={{.data._raw}}
  | base64 -d
  | from json 
  | get data 
  | transpose k v 
...
alias claude = mise exec node@20 -- claude --dangerously-skip-permissions
alias opus = mise exec node@20 -- claude --dangerously-skip-permissions --model opus
alias sonnet = mise exec node@20 -- claude --dangerously-skip-permissions --model sonnet
alias haiku = mise exec node@20 -- claude --dangerously-skip-permissions --model haiku
